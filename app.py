...
async def handler(event):
    try:
        ...
        sender_id = sender.id
        username = sender.username if sender.username else 'unknown'  # Gestione username mancante
        ...
        if event.message.message and event.message.media:
            ...
            if file_name:  # Se il nome del file è valido
                ...
                save_message(base_folder, folder_name, event.message.message, sender_id, username, f'{media_type.capitalize()} con testo', timestamp, file_path)
            else:
                save_message(base_folder, folder_name, event.message.message, sender_id, username, 'text', timestamp)
        ...
        # Gestione solo media
            ...
            if file_name:  # Se il nome del file è valido
                file_path = await event.message.download_media(file=f'./chats/{chat_type}/{folder_name}/{media_type}/{file_name}')
                save_message(base_folder, folder_name, media_type.capitalize(), sender_id, username, media_type, timestamp, file_path)
            else:
                save_message(base_folder, folder_name, 'Unknown Media', sender_id, username, 'unknown', timestamp)
